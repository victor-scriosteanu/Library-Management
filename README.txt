...
My program represents an application with a simple management system for the librarian to use. An esential requirement would be to quit the app and restart it if a withdrawn or return was made in order to update all the new data.

It provides an interface with a menu with 5 options to chose from:

-> Search for book:
    -has a little search engine which can recognize a book title, only by entering a letter(doesn't matter the case) which is contained in the title;
...
-> Check for available books:
    ...
    -gives the user the option to withdrawn a book; 
    -checks from database if the last element from every book is alocated to an student ID and displays Not available if is equal to 0 it displays Available;
...
-> Access loan history:
    -provides a list with all books and it's informations that were withdrawn for a period longer than 30 days;
...
With the information about reseting the app i mentioned earlier, please feel free to navigate as i putted enough books in the databse with a variation of names and availabilities. 
